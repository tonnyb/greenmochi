...
class socketDaemon {
	...
	public function socket_select(&$read_set, &$write_set, &$exception_set, $timeout) {
	  return @socket_select($read_set, $write_set, $exception_set, $timeout);
	}
	...
	public function process($count = -1)
	{
		...
		$event_time    = time();
		while ($this->count != 0 && ($events = $this->socket_select($read_set, $write_set, $exception_set, 2)) !== false) {
