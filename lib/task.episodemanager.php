...
import('task');
class TaskEpisodeManager extends Task {
	public function run() {
		parent::run();
		sleep(1);
	...
	public function runManager() {
		...
		foreach ( $shows as $show ) {
			...
			$newepisodes = EpisodeFactory::getEpisodes(array('showid' => $show->getShowId(), 'airdate' => array($airdateFrom, $airdateTo), '!want' => 2, 'file' => ''));
			$wantedepisodes = EpisodeFactory::getEpisodes(array('showid' => $show->getShowId(), 'want' => 1, 'file' => ''));
	...
	private function needEpisode($show, $episode, $items) {
		$epreg = '(?:[ \-_.]+)' . 
			'(?:[sS](?<season>[0-9]+))?' . 
			'(?:[eE][pP](?:isode)?)?' . 
			'(?<episode>[0-9]+\.[0-9]+|[0-9]+)';
	...
	private function parseTitle($itemTitle, $titles) {
	...
	private function getParsedTitles($show) {
