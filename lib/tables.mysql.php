...
$tables = array(
	'shows' => 'CREATE TABLE IF NOT EXISTS shows (showid int not null primary key auto_increment, title varchar(100) not null, name varchar(100) not null, episodes tinyint(4) not null, path text not null, site varchar(20) not null, network varchar(20), quality varchar(10), startdate date not null, enddate date not null, changedate date not null, download int not null, scan int not null, aid int not null, acid int not null, unique(name)) CHARACTER SET utf8 COLLATE utf8_unicode_ci engine=InnoDB;',
